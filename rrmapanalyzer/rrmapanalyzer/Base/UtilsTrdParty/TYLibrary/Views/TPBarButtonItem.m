...
// <
+ (UIBarButtonItem *)backItemWithoutTitle:(id)target action:(SEL)action {
    UIBarButtonItem *leftBackItem = [[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStylePlain target:target action:action];
    leftBackItem.image = [UIImage imageNamed:@"icon_back"];
    return leftBackItem;
}
...
+ (UIBarButtonItem *)backItem:(id)target action:(SEL)action {
    ...
    leftBackItem.image = [UIImage imageNamed:@"icon_back"];  // tp_top_bar_back
    return leftBackItem;
}
