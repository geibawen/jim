...
{
    // 处理结果
    NSMutableArray *transData = [NSMutableArray new];
    NSLog(@"updatedTransactions count:%d", transactions.count);
...
//                NSLog(@"receiptStr:%@", receiptStr);
                [[RRUser sharedInstance] postBuyReceipt:encodeStr success:^(){
                    NSLog(@"check receipt ok from server when start. new expire:%ld", [RRUser sharedInstance].expireDate);
                    if ([self.delegate respondsToSelector:@selector(IAPToolBoughtProductSuccessedWithProductID:andInfo:)]) {
                        ...
                        //没有代理说明不是购买页面收到的receipt，提交服务端后直接finish
                        NSLog(@"Submited auto renewed receipt");
                    }
                    [[SKPaymentQueue defaultQueue] finishTransaction:transaction];
                } failure:^(NSError *error){
                    NSLog(@"check receipt failed on fv server");
...
//    [self.delegate IAPToolBoughtProductSuccessedWithProductID:ProductID receipt:encodeStr];
    ...
    return; //这里不做客户端验证，直接返回凭据，认为成功，交给服务器.
